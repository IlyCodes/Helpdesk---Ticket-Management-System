...
<div class="bg-white shadow-md overflow-hidden sm:rounded-lg">
        ...
        <div class="flex flex-col sm:flex-row items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    ...
                    <span class="font-medium">{{ $ticket->agent ? $ticket->agent->name : 'Not Assigned' }}</span>
                    @if(!$ticket->agent || $ticket->status->slug === 'not-assigned')
                <form action="{{ route('admin.tickets.assignWithAI', $ticket) }}" method="POST" class="inline-block ml-2">
                    @csrf
                    @method('PATCH') {{-- Or POST, depending on how you define the route --}}
                    <button type="submit" class="px-2 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50"
                        onclick="return confirm('Are you sure you want to try assigning this ticket with AI?');">
                        Assign with AI
                    </button>
                </form>
                @endif
            ...
            <div class="flex items-center space-x-2">
                {{-- Download PDF Button --}}
                <a href="{{ route('admin.tickets.pdf', $ticket) }}" target="_blank"
                    ...
                    {{-- SVG Icon for PDF --}}
                    <svg class="w-4 h-4 mr-2" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-0">
        <div class="md:col-span-2 border-r-0 md:border-r border-gray-200">
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex gap-x-3 {{ $reply->user_id === Auth::id() ? 'flex-row-reverse' : '' }}">
                        ...
                        <div class="flex-auto rounded-md p-3 ring-1 ring-inset {{ $reply->user_id === Auth::id() ? 'bg-purple-50 ring-purple-200' : ($reply->is_internal ? 'bg-yellow-50 ring-yellow-300' : ($reply->user->isClient() ? 'bg-blue-50 ring-blue-200' : 'bg-gray-50 ring-gray-200')) }}">
                            <div class="flex justify-between items-baseline gap-x-2">
                                <p class="text-sm font-semibold 
                                            @if($reply->user_id === Auth::id()) text-purple-700 
                                            @elseif($reply->user->isClient()) text-blue-700 
                                            @elseif($reply->user->isAgent()) text-red-600
                                            @else text-gray-900 @endif">
                                    {{ $reply->user->name }}
                                    @if($reply->user->isClient()) <span class="text-xs font-normal text-blue-500">(Client)</span> @endif
                                    @if($reply->user->isAgent()) <span class="text-xs font-normal text-red-500">(Agent)</span> @endif
                                    @if($reply->user->isAdmin() && $reply->user_id !== Auth::id()) <span class="text-xs font-normal text-purple-500">(Admin)</span> @endif
                                </p>
        ...
        <div class="md:col-span-1 p-6 space-y-6 bg-gray-50 md:bg-white">

            <div>
                <h3 class="text-lg font-medium text-gray-900 mb-3">Assign Agent</h3>
                <form action="{{ route('admin.tickets.assignAgent', $ticket) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        <label for="agent_id" class="sr-only">Agent</label>
                        <select name="agent_id" id="agent_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2">
                            <option value="">Select an agent...</option>
                            @foreach($agents as $agentOption)
                            <option value="{{ $agentOption->id }}" {{ $ticket->assigned_agent_id == $agentOption->id ? 'selected' : '' }}>
                                {{ $agentOption->name }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div>
                ...
                <form action="{{ route('admin.tickets.updateStatus', $ticket) }}" method="POST">
                    ...
                    @method('PATCH')
                    <div class="mb-3">
                        <label for="status_id_admin" class="sr-only">Status</label>
                        <select name="status_id" id="status_id_admin" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2">
            ...
            <div>
                ...
                <form action="{{ route('admin.tickets.reply', $ticket) }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label for="body_admin" class="sr-only">Reply Body</label>
                        <textarea name="body" id="body_admin" rows="5" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Type your message here..."></textarea>
                        @error('body') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="is_internal_admin" class="inline-flex items-center">
                            <input type="checkbox" name="is_internal" id="is_internal_admin" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            @if($ticket->assignmentLogs->isNotEmpty())
            <hr class="my-6">
            <div>
                <h3 class="text-lg font-medium text-gray-900 mb-3">Assignment History</h3>
                <ul class="space-y-2 text-sm text-gray-600">
                    @foreach($ticket->assignmentLogs as $log)
                    <li class="p-2 border rounded-md bg-gray-100">
                        Assigned to <span class="font-semibold">{{ $log->assignedAgent->name }}</span>
                        by <span class="font-semibold">{{ $log->admin->name }}</span>
                        on {{ $log->assigned_at->format('M d, Y H:i') }}.
                        @if($log->previousAgent)
                        <span class="block text-xs text-gray-500">Previously: {{ $log->previousAgent->name }}</span>
                        @else
                        <span class="block text-xs text-gray-500">Previously: Unassigned</span>
                        @endif
                    </li>
                    @endforeach
                </ul>
    ...
    <div class="px-4 py-3 bg-gray-50 text-right sm:px-6 border-t border-gray-200">
        <a href="{{ route('admin.tickets.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            Back to All Tickets
        </a>
    </div>
</div>
